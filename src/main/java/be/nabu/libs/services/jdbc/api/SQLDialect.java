...
//								statement.setObject(index++, collection.toArray((Object[]) newInstance), sqlType);
	...
	public static String getDefaultTotalCountQuery(String query, boolean useStarCount) {
		...
		if (isGrouped(query) || isUnion(query) || isDistinct(query)) { //  || isPotentialCTEDistinct(query) (not yet, regression impact analysis?)
...
//		return "select count(a.*) as total from (" + query + ") a";
	...
	public static boolean isPotentialCTEDistinct(String sql) {
		// specific workaround for CTEs
		if (sql.startsWith("with") && sql.matches("(?i)(?s).*\\bselect[\\s]+distinct[\\s]+.*")) {
			return true;
		}
		return false;
	}
